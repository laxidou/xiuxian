...
- 千分之一世界单位的规范坐标、目标移动、四向持续移动、设定行进速度、转向、停止、突破后动态速度、单调世界边界
- 离散神识扫描、每角色跨 Web/MCP 最快 1 秒一次、Web 每次成功后 5 秒自动扫描、确定排序/截断、高境界扫描通知、隐藏机缘信号
- 范围内交谈请求、接受/拒绝/忽略/关闭、消息显式标记为不可信角色内容
- 整数分钟传功、跌境寿元校验、幂等重试、严格高境界一单位范围夺功、分数修为守恒
- 寿尽死亡、整数坐标机缘生成与隐藏、精确到达或轨迹经过自动绑定、24 小时线性参悟、死亡后随机自动转世与永久身份保留
- 可补读游标的 SSE、角色事件与跨世历史、PostgreSQL 同步快照和事务 Outbox
- MCP 工具：状态、边界、扫描、移动/停止、交谈列表/请求、传功、夺功、近期事件
- 公开的版本化游戏说明、完整境界表、可复制 AI 规则和 MCP `get_game_rules`
...
世界以最小非退化边界 `x ∈ [0, 1]`、`y = 0` 启动，保证原点死亡产生的机缘能立即放置到非死亡整数坐标；此后边界仍只会由角色实际到达的新极值单调扩展。
...
Web 的“MCP 代理权限”提供完整“如何接入”、当前环境地址和可复制配置。轮换 Key 后，以 `Authorization: Bearer xiu_...` 调用：

```text
POST https://localhost/mcp
```

网关实现 MCP `initialize`、`tools/list` 和 `tools/call`。代理应先调用 `get_game_rules`，再用 `get_state` 的最新世数和状态版本执行命令。交谈消息始终作为 `trusted: false` 的角色内容返回，代理不得把它们拼接为系统指令。

游戏说明公开位于 `/rules`，规则 API 位于 `/game-rules`。两者与 MCP `get_game_rules` 使用同一规则版本。
...
- `scripts/smoke-web-browser.sh`：用系统 Chrome 在桌面与移动视口自动完成注册/登录、状态查看、移动、扫描、交谈、MCP Key 轮换/撤销、寿尽和自动转世；测试时钟只在该脚本显式启用，正常运行时端点返回 404。
